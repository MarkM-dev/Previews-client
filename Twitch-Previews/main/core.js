...
(async () => {
    ...
    function createTooltip(el, position, text) {
        el.classList.add('tp-tooltip-container');
        let tooltip = document.createElement('span');
        tooltip.classList.add('tp-tooltip');
        tooltip.classList.add('tp-tooltip-pos-' + position);
        tooltip.innerText = text;

        let old_tooltip = el.querySelector('.tp-tooltip');
        if (old_tooltip) {
            el.replaceChild(tooltip, old_tooltip);
        } else {
    ...
    function handleCustomSidebarHeader(followed_channels_section, dataObj) {
        ...
        if (!isNavBarCollapsed) {
            ...
            let title_el_text_el = favorites_section.querySelector('h2') || favorites_section.querySelector('h5');
            if (title_el_text_el) {
                title_el_text_el.innerText = dataObj.section_title_text;
            }

            let isTTVSortableSidebar_title_el = !!favorites_section.querySelector('.followed-side-nav-header--expanded');
            if (isTTVSortableSidebar_title_el) {
                try {
                    title_el_text_el.parentNode.querySelector('p').remove();
                    title_el_text_el.parentNode.nextSibling.remove();
                    favorites_section.querySelector('button.tw-interactable').classList.add('tp-custom-sidebar-sortable-header');
                } catch (e) {
            ...
            if (streamers_arr.length > 0) {
                ...
                if (shown_followed_channels[0]) {
                    ...
                    for (let i = 0; i < streamers_arr.length; i++) {
                        ...
                        append_container.appendChild(container_div);
                    }
                }
            }

            if(!isExperimentalSidebar) {
                if (!append_container.firstChild && !isNavBarCollapsed) {
    ...
    function appendFavoritesBtn() {
        ...
        try {
            ...
            if(bell_btn) {
                ...
                favorites_btn_tooltip.style.width = 'max-content';
    ...
    function createSidebarFavoritesElement(followed_streamer_element, style, offline_stream_name, offline_display_name, profile_pic_url) {
        ...
        if (offline_stream_name) {
            ...
            el.addEventListener('mouseover', (e) => {
                ...
                style.textContent = 'div[data-popper-escaped][data-popper-placement="right-start"] {transform: none !important;inset: 0px auto auto ' + distance + ' !important;' + (isLayoutHorizontallyInverted ? "right":"left") + ': ' + distance + ' ;top: ' + el.getBoundingClientRect().top + 'px !important;} div[data-popper-escaped][data-popper-placement="right-start"] .online-side-nav-channel-tooltip__body + div{display:none !important}';
            });
    ...
    function setfScrnWithChatBtn() {
        ...
        try {
            ...
            if (append_containers.length) {
                ...
                createTooltip(custom_chat_btn, 'top-left', _i18n('fScrnWithChat_custom_chat_btn_title'));
                menu_div.appendChild(custom_chat_btn);
                if (!isInIframe()) {
                    createTooltip(default_chat_btn, 'top-right', _i18n('fScrnWithChat_default_chat_btn_title'));
...
*/
    ...
    function append_MultiStream_btn() {
        ...
        try {
            ...
            if (more_btn) {
                ...
                btn_container_tooltip.style.bottom = '0';
    ...
    function showToast(toast_body, storageFlagName, toastType) {
        ...
        updateToast.innerHTML = "<div style=\"font-size: 14px;color: white;\" >\n" +
            ...
            "                        0-1.019-2.588,321.173,321.173,0,0,1-45.868-21.853,1.885,1.885,0,0,1-.185-3.126c3.082-2.309,6.166-4.711,9.109-7.137a1.819,1.819,0,0,1,1.9-.256c96.229,43.917,200.41,43.917,\n" +
            "                        295.5,0a1.812,1.812,0,0,1,1.924.233c2.944,2.426,6.027,4.851,9.132,7.16a1.884,1.884,0,0,1-.162,3.126,301.407,301.407,0,0,1-45.89,21.83,1.875,1.875,0,0,0-1,2.611,391.055,\n" +
            "                        391.055,0,0,0,30.014,48.815,1.864,1.864,0,0,0,2.063.7A486.048,486.048,0,0,0,610.7,405.729a1.882,1.882,0,0,0,.765-1.352C623.729,277.594,590.933,167.465,524.531,69.836ZM222.491,\n" +
            "                        337.58c-28.972,0-52.844-26.587-52.844-59.239S193.056,219.1,222.491,219.1c29.665,0,53.306,26.82,52.843,59.239C275.334,310.993,251.924,337.58,222.491,337.58Zm195.38,\n" +
            "                        0c-28.971,0-52.843-26.587-52.843-59.239S388.437,219.1,417.871,219.1c29.667,0,53.307,26.82,52.844,59.239C470.715,310.993,447.538,337.58,417.871,337.58Z\"></path></svg></div>\n" +
            "               <div id='tp_updateToast_twitter_btn' class='tp_updateToast_social_btn' ><svg style='padding-bottom: 1px;' stroke=\"currentColor\" fill=\"currentColor\" stroke-width=\"0\" width=\"13px\" height=\"16px\" viewBox=\"0 0 512 512\" xmlns=\"http://www.w3.org/2000/svg\">\n" +
            "                            <path d=\"M459.37 151.716c.325 4.548.325 9.097.325 13.645 0 138.72-105.583 298.558-298.558 298.558-59.452 0-114.68-17.219-161.137-47.106 8.447.974\n" +
...
"                            </div>\n" +
    "                       <div id=\"tp_updateToast_donate_bitcoin_btn\" class=\"tp_updateToast_donate_btn\" style=\"width: 14%;\" >\n" +
    ...
    "                                    <path d=\"M504 256c0 136.967-111.033 248-248 248S8 392.967 8 256 119.033 8 256 8s248 111.033 248 248zm-141.651-35.33c4.937-32.999-20.191-50.739-54.55-62.573l11.146-44.702-27.213-6.781-10.851\n" +
    "                                    43.524c-7.154-1.783-14.502-3.464-21.803-5.13l10.929-43.81-27.198-6.781-11.153 44.686c-5.922-1.349-11.735-2.682-17.377-4.084l.031-.14-37.53-9.37-7.239 29.062s20.191 4.627 19.765 4.913c11.022\n" +
    "                                    2.751 13.014 10.044 12.68 15.825l-12.696 50.925c.76.194 1.744.473 2.829.907-.907-.225-1.876-.473-2.876-.713l-17.796 71.338c-1.349 3.348-4.767 8.37-12.471 6.464.271.395-19.78-4.937-19.78-4.937l-13.51\n" +
    "                                    31.147 35.414 8.827c6.588 1.651 13.045 3.379 19.4 5.006l-11.262 45.213 27.182 6.781 11.153-44.733a1038.209 1038.209 0 0 0 21.687 5.627l-11.115 44.523 27.213 6.781 11.262-45.128c46.404 8.781 81.299 5.239\n" +
    "                                    95.986-36.727 11.836-33.79-.589-53.281-25.004-65.991 17.78-4.098 31.174-15.792 34.747-39.949zm-62.177 87.179c-8.41 33.79-65.308 15.523-83.755 10.943l14.944-59.899c18.446 4.603 77.6 13.717 68.811 48.956zm8.417-87.667c-7.673\n" +
    "                                    30.736-55.031 15.12-70.393 11.292l13.548-54.327c15.363 3.828 64.836 10.973 56.845 43.035z\"></path>\n" +
    "                                </svg>\n" +
    "                            </div>" +
            "            </div>\n" +
        ...
        updateToast.querySelector('#tp_updateToast_donate_bitcoin_btn').onclick = function () {
            sendMessageToBG({action: "bg_show_donate_bitcoin", detail: ""});
            sendMessageToBG({action: toastType + "_donate_bitcoin_btn_click", detail: ""});
        };
...
/*            +  "       <div style=\"font-size: 14px;color: white;margin-top: 20px;\" ><strong style='color: #2cff95;' >- Fixes and improvements</strong>"
            ...
            +  "             <div style='height: 8px;' > </div>"
            +  "             <span style=\"font-size: 12px;color: whitesmoke;font-weight: bold;\" >- Fixed mature content warning in previews.</span>"
            +  "             <br><span style=\"font-size: 12px;color: whitesmoke;\" >You can now hover the preview and approve the mature content message.</span>"
            +  "       </div>"*/
         /*   +  "    </br>"*/
    }
    ...
    function settings_FBsidebar_cb_off() {
        let settingsContainer = document.getElementById('TPBodyEl');
        if (settingsContainer) {
            settingsContainer.querySelector('#TP_popup_FBsidebar_checkbox').checked = false;
        }
    }

    function settings_FBsidebar_cb_on() {
        let settingsContainer = document.getElementById('TPBodyEl');
        if (settingsContainer) {
            settingsContainer.querySelector('#TP_popup_FBsidebar_checkbox').checked = true;
        }
    }
                    ...
                    if (selectedSettingsForImportObj.selectedSettings.includes("tp_options")) {
                        _browser.runtime.sendMessage({action: "tp_settings_upgrade_db", detail: loaded_settings.tp_options}, function(response) {
                            setSettingsToStorage('tp_options', response.result.upgraded_options).then(function (res) {
                                ...
                                if (loaded_settings.tp_options.isFBsidebarEnabled) {
                                    new_permissions_ask_arr.push('isFBsidebarEnabled');
                                }
    ...
    function initFBstreamersDialog(settingsContainer) {
        let fb_streamers_input_container = settingsContainer.querySelector('#fb_streamers_input_container');
        let fb_streamers_input = settingsContainer.querySelector('#fb_streamers_input');
        let fb_streamers_save_btn = settingsContainer.querySelector('#fb_streamers_save_btn');
        ...
        let err_div = settingsContainer.querySelector('#tp_settings_fb_streamers_popup_err');
        ...
        function createStreamerTagSpan(text) {
            let span = document.createElement('span');
            span.classList.add('tp-settings-fb-streamers-tag-span');

            let name_span = document.createElement('span');
            name_span.innerText = text;

            let delete_btn = document.createElement('span');
            delete_btn.classList.add('tp-settings-fb-streamers-tag-delete_btn');
            delete_btn.innerText = 'x';
        ...
        fb_streamers_input_container.onclick = (e) => {
            fb_streamers_input.focus();
        }

        function validate_FBstreamersInput_spacesAndLinks(streamer_name) {
            if (streamer_name.indexOf(' ') > -1 || streamer_name.indexOf('/') > -1) {
        ...
        fb_streamers_input.addEventListener('keyup', (e) => {
            ...
            if (key === 32 || key === 188) {
                ...
                streamer_name = streamer_name.slice(0, -1);
                if (!validate_FBstreamersInput_spacesAndLinks(streamer_name)) {
                    fb_streamers_input.value = "";
                    return;
                }

                let span = createStreamerTagSpan(streamer_name);
        ...
        fb_streamers_cancel_btn.onclick = (e) => {
            hideFBstreamersEditor();
        }

        function populateSelectedStreamersBox() {
            _browser.storage.local.get('fb_streamers', function(result) {
                if (result.fb_streamers && result.fb_streamers.length > 0) {
                    for (let i = 0; i < result.fb_streamers.length; i++) {
                        let span = createStreamerTagSpan(result.fb_streamers[i]);
                        fb_streamers_input.before(span);
                    }
                }
                handleInputPlaceholderVisibility();
        ...
        function clearSelectedStreamersBox() {
            let selected_streamers_spans = fb_streamers_input_container.querySelectorAll('.tp-settings-fb-streamers-tag-span');
            for (let i = 0; i < selected_streamers_spans.length; i++) {
        ...
        function hideFBstreamersEditor() {
            settingsContainer.querySelector('#tp_settings_fb_streamers_popup').classList.add('tp_display_none');
            clearSelectedStreamersBox();
        }

        edit_btn.onclick = (e) => {
            if (settingsContainer.querySelector('#tp_settings_fb_streamers_popup').classList.contains('tp_display_none')) {
                showFBstreamersEditor();
            } else {
                hideFBstreamersEditor();
            }
        }
